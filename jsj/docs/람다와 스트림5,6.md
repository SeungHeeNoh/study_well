...
### W5) 
```  java
IntBinaryOperator test = Integer::compare; //BiFunction<Integer, Integer, Integer> or BinaryOperator<Integer> are also available
System.out.println(test.applyAsInt(1, 1)); //apply instead of applyAsInt
```
----
### Q6) 
```  
...
### W9) 
```  java
UnaryOperator<String> test = String::toUpperCase;  // Function<String, String>
System.out.println(test.apply("hello world"));
```
----
### Q10) 
...
```
```  java
System.out.println( Arrays.stream(strArr).reduce("", String::concat).length() );
```
----
 <br/> <br/>
...
```
```  java
System.out.println( Arrays.stream(strArr).mapToInt(String::length).max().getAsInt() );
```